...
def calculate_score(cards):
    """Take a list of cards as input and return the score calculated from cards"""
...
def compare(user_score, computer_score):
    if user_score == computer_score:
        ...
        return "You went over 21 so lose"
...
#the main game part is from hint 5 to hint 13 so we will create a new function called play_game so we can use it for hint 14 to restart
def play_game():
    #Hint 5: Deal the user and computer 2 cards each using deal_card() and append().
    user_cards = []
    computer_cards = []
    is_game_over = False #creating a boolean here to use for hint 9

    for _ in range(2):   # the 2 inside range is to run the loop twice ..also instead of "_" I can even use num for example
    ...
    while not is_game_over:
        ...
        print(f" Computers first card: {computer_cards[0]}")  #computers first card because dealer will reveal computers first card
    ...
    while computer_score != 0 and computer_score < 17:  # 0 represents blackjack but we dont wanna draw a card if he has blackjack thatswhy != 0
        computer_cards.append(deal_card())
        computer_score = calculate_score(computer_cards)

    # calling the compare function from hint 13
    print(f"Your final hand: {user_cards}, final score {user_score}")# adding extra print statements so everything is more clear in console while playing
    print(f"Computers final : {computer_cards}, final score {computer_score}")
